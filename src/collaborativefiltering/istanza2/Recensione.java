	...
	public static void recItems(String c, List<String>film, Map<String, List<Recensione>>map)throws Exception{
        ...
        for (int j = 0; j < film.size(); j++) {
	        ...
	        for (String name : map.keySet()) {
		      ...
		      System.out.println(v+", "+film.get(j));
              if(v > 0){
			   double sumprov = 0; 
			   sumprov = Recensione.pearson(c, name, map)*v;
			   System.out.println("il risultato tra "+c+", "+name+" è "+sumprov);
			   
			   sum += sumprov;
			   sumsim += Recensione.pearson(c, name, map);
		      }
            }
	        System.out.println("il valore di sum è "+sum+" e il valore di sumsim è "+sumsim);
	       sc_pes = sum/sumsim;
